...
describe("The webpage", () => {
  ...
  describe("tagline", () => {
    ...
    it("should have a non-empty h3 tag @tagline-content", () => {
      ...
      assert.isOk(
        h3.textContent,
        "Our tagline's `h3` element cannot be empty."
      );
    });
  ...
  describe("skills", () => {
    ...
    it("should have at least 3 skills @skills-list", () => {
      ...
      assert.isAtLeast(
        skillItems.length,
        3,
        "We need at least 3 `li` elements inside the skills' `ul`."
      );
    });
    ...
    it("should have one skill that contains HTML @skills-list", () => {
      ...
      assert.equal(
        skillsWithHtml.length,
        1,
        "HTML needs be part of one of your skills."
      );
    });
  });
  ...
  describe("contact", () => {
    ...
    it("should have a non-empty h3 tag @contact-content", () => {
      ...
      assert.isOk(
        h3.textContent,
        "Our contact's `h3` element cannot be empty."
      );
    });
    ...
    it("should have a link with an href within the paragraph @contact-link", () => {
      ...
      assert.isOk(
        a.getAttribute("href"),
        "Our contact's `a` element needs a non-empty `href` attribute."
      );
    });
  });
});
